 ...
 */
 // One of the drone's firmware files, controlling UART
 // Only uart1Test, uart1Getchar were modified
#include <string.h>
...
bool uart1Test(void)
{
	// Blink LEDs to verify function is being executed and UART1 enabled
	for (int i = 0; i < 5; i++) {
		ledSet(3, 0);
		vTaskDelay(M2T(200));
		ledSet(3, 1);
...
// Waiting until there is an incoming UART message
// then read the next char in queue and return the number of chars left
