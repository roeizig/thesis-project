...
// Roei: Included for DEBUG_PRINT; ledTest (Both for debugging)
#include "debug.h"
#include "led.h"
#include "task.h"
#include "system.h"
...
void uart1Init(const uint32_t baudrate)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = NVIC_HIGH_PRI; // Roei: Changed from MID --> HIGH
...
bool uart1Test(void)
{
	// Roei: Function is being executed
	for (int i = 0; i < 5; i++) {
		ledSet(3, 0);
		vTaskDelay(M2T(200));
		ledSet(3, 1);
	...
	consolePrintf("CP Enabling UART1\n");
	return isInit;
}
...
//	DEBUG_PRINT("after Queue\n");
    return (int) uxQueueMessagesWaiting(uart1queue);
}
...
void __attribute__((used)) USART3_IRQHandler(void)
{
  ...
  while (USART_GetITStatus(UART1_TYPE, USART_IT_RXNE))
  {
    rxData = USART_ReceiveData(UART1_TYPE) & 0x00FF;
    xQueueSendFromISR(uart1queue, &rxData, &xHigherPriorityTaskWoken);
    /*Roei: Added TaskNotify to signal uart data received and execute spin.
    //		Check whether TaskNotify is needed, or QueueSend is enough!
    //		portYIELD_FROM_ISR is needed to preform the desired context switch
    */
    //vTaskNotifyGiveFromISR(xSpinHandle, &xHigherPriorityTaskWoken);
